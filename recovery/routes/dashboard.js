    ...
    try {
        ...
        if (msgRows && msgRows.length > 0) {
            msgRows.forEach(row => {
                ...
                totalMessages += count;
                if (row.type === 'SMS') smsCount += count;
                else if (row.type.includes('알림톡') || row.type === 'D3톡') alimtalkCount += count;
...
router.get('/dashboard/patient/:chartNo', async (req, res) => {
    try {
        ...
        const decodedChartNo = decodeURIComponent(chartNo);
        ...
        // 4. 처방 이력 (prescriptions 테이블 삭제됨)
        // const [prescriptions] = await pool.query(
        //     "SELECT id, patientId, patientName, patientChartNo, items, doseInfo, purpose, DATE_FORMAT(decoctionDate, '%Y-%m-%d') as decoctionDate, createdAt FROM prescriptions WHERE patientId = ? ORDER BY createdAt DESC LIMIT 20",
        //     [patient.id]
        // );
        const prescriptions = [];
