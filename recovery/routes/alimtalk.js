...
router.post('/alltalk/import-sens-templates', async (req, res) => {
    try {
        ...
        res.json({ 
            success: true, 
            message: `동기화 완료: ${newCount}건 추가, ${updatedCount}건 업데이트됨.`, 
            sensTemplates 
        });
    } catch (error) {
...
router.post('/alltalk/send-alimtalk', async (req, res) => {
    try {
        const { patients, templateCode, content, buttons, variables } = req.body;
        const results = [];
        ...
        for (const p of patients) {
            try {
                ...
                await pool.query(
                    ...
                    VALUES (?, ?, ?, ?, NOW(), '알림톡', ?, ?, ?, ?)`,
                    [Date.now() + Math.round(Math.random() * 1000), p.name, p.chartNo || '', formattedPhone, statusName, templateCode, errorDesc, requestId]
                );
...
router.post('/send-sms', async (req, res) => {
    const { patients, content } = req.body;
    if (!patients || !content) return res.status(400).json({ success: false, message: '보낼 환자와 내용을 입력하세요.' });
    ...
    try {
        ...
        for (const p of patients) {
            ...
            await pool.query(
                ...
                VALUES (?, ?, ?, ?, NOW(), ?, ?, ?)`,
                [Date.now() + Math.round(Math.random() * 100), p.name || '', p.chartNo || '', formattedPhone, sentType, result.success ? 'success' : 'failed', content]
            );
...
router.post('/log-message', async (req, res) => {
    try {
        const { patientName, chartNo, phone, type, status, message } = req.body;
