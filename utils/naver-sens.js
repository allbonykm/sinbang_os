...
function getAutoConfig() {
    return {
        ...
        BMM_SERVICE_ID: process.env.BMM_SERVICE_ID,
...
// 4바이트 이상의 특수 이모지 완벽 제거 (모든 Surrogate Pair 제거)
function stripEmojis(str) {
    if (!str) return '';
    // 모든 4바이트 문자(이모지 등)를 제거하는 가장 강력한 정규식
...
async function sendBrandMessage(name, chartNo, phone, templateCode, messageType = 'TEXT', customContent = null, customButtons = null, variables = null) {
    ...
    const timestamp = Date.now().toString();
    const urlPath = `/brandmessage/v2/services/${config.BMM_SERVICE_ID}/messages`;
    const signature = makeSignature('POST', urlPath, timestamp);

    let bmmContent = '';
    let buttons = customButtons;

    const loadedTemplate = loadTemplateFromJson(templateCode);
    const vars = variables || {};

    if (customContent) {
        bmmContent = customContent
            .replace(/#\{(이름|name)\}/g, name)
            .replace(/#\{(date|예약날짜|예약일시)\}/g, (vars && (vars.date || vars.예약날짜 || vars.예약일시)) || '#{date}')
            .replace(/\$\{name\}/g, name);
        ...
        Object.keys(vars).forEach(key => {
            const regex = new RegExp(`#\\{${key}\\}`, 'g');
            bmmContent = bmmContent.replace(regex, vars[key] || '');
        });
    } else if (loadedTemplate) {
        const templateContent = loadedTemplate.sensContent || '';
        if (templateContent) {
            bmmContent = templateContent
                .replace(/#\{(이름|name)\}/g, name)
                .replace(/#\{(date|예약날짜|예약일시)\}/g, (vars && (vars.date || vars.예약날짜 || vars.예약일시)) || '#{date}')
                .replace(/#\{차트번호\}/g, chartNo || '')
                .replace(/\$\{name\}/g, name);
    ...
    if (customButtons) {
        buttons = customButtons.map(btn => ({
            type: btn.type || 'WL',
            name: btn.name,
            linkMobile: (btn.linkMobile || '').replace(/#\{이름\}/g, encodeURIComponent(name)).replace(/#\{차트번호\}/g, encodeURIComponent(chartNo || '')),
            linkPc: (btn.linkPc || '').replace(/#\{이름\}/g, encodeURIComponent(name)).replace(/#\{차트번호\}/g, encodeURIComponent(chartNo || ''))
        }));
    }
    ...
    const payload = {
        ...
        targeting: 'M', // 채널 친구 여부와 상관없이 발송 (고객사 수신동의 유저)
        isAdult: false,
        messages: [{
            to: phone,
...
// 브랜드 메시지 템플릿 조회
async function getBrandMessageTemplates(templateCode = null) {
    const config = getConfig();
    if (!config.BMM_SERVICE_ID) {
        return { success: false, error: 'BMM_SERVICE_ID가 설정되지 않았습니다. .env 파일에 네이버 SENS Brand Message 프로젝트 ID를 추가해 주세요.' };
    }
    const timestamp = Date.now().toString();
    let urlPath = `/brandmessage/v2/services/${config.BMM_SERVICE_ID}/templates`;
    if (templateCode) urlPath += `?templateCode=${encodeURIComponent(templateCode)}`;
